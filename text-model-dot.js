...
function report_svg(path, tree, _reports) {
  ...
  for (const t in tree) {
    if (tree?.[t]?.role?.[role] || role == "All") {
      rep.push(
        `${Object.keys(tree[t].last_name)[0]}, ${
          Object.keys(tree[t].first_name)[0]
        }${t}`,
      );
    }
  }
  ...
  let bar = 1;
  let pg =
    `<text font-family="Arial" font-weight="bold" font-color="black" font-size="18px" x="10px" y="16px">${role}</text>
  ...
  for (const j of rep) {
    ...
    if (0 < bar && bar < 4) {
      pg += `<rect x="5px" y="${
        i - 11
      }px" width="700px" height="15px" fill="#d1eee1"></rect>`;
    }
    pg +=
      `<text font-family="Arial" font-color="black" font-size="12px" x="10px" y="${i}px">${
        j.slice(0, -8)
      }<tspan x="170px" y="${i}">${
        Object.keys(tree[id].street_address)[0]
      }</tspan>
      <tspan x="400px" y="${i}">${
        Object.keys(tree[id].mobile_number)[0]
      }</tspan>
      <tspan text-anchor="end" x="700px" y="${i}">${
        Object.keys(tree[id].first_name)[0]
      }.${Object.keys(tree[id].last_name)[0]}@example.org</tspan></text>
      `;
    bar++;
    if (bar == 6) bar = 0;
    pg += "</g>";
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="705px" height="${
    i + 10
  }px">\n<rect width="100%" height="100%" fill="white"></rect>\n ${pg}</svg>`;
}
function n_s(nd) {
  return nd == "transforms"
    ? {
      marg: ".01",
      fill: "#ffd6bc",
      col: "#EE7733",
      ...
      if (line.includes("::")) {
        ...
        if (last_command != "level") {
          if (
            ...
            edge_color =
              ["entities", "processes", "datastores"].includes(last_command)
                ? "#cc3311"
                : "#009988";
            const ns = n_s(last_command);
            const lbl = l_lvl in reports && !(l_lvl in dots)
              ? ` href="#${l_lvl}" tooltip="#${l_lvl}" label="#\n ${l_lbl} "`
              : l_lvl in dots && last_command == "agents"
              ? ` href="#${l_lvl}" tooltip="#${l_lvl}" label=" ${l_lbl} "`
              : l_lvl in dots
              ? ` href="#${l_lvl}" tooltip="#${l_lvl}" label="${l_lbl}"`
              : `label="${l_lbl}"`;
            dots[level.join(".")][line] =
            ...
            if (["forward", "back", "both"].includes(last_command)) {
              ...
              dots[level.join(".")][
                ...
                `"${last_subject}" -> "${line}" [color="${edge_color}" dir="${last_predicate}"]`;
              structured.push({ x: 2, y: row, w: 2, h: 1, content: line });
            } else {
              if (["items"].includes(last_command)) {
                items.push(line);
...
export { model_to_dots, model_to_dots_ii, n_s, report_svg };
